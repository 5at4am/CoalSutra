...
- `templates.py` — 3 templates: `production_summary`, `coal_quality_report`,
  `reserve_estimate`; each declares section order + per-section drafting instructions.
- `generator.py` — drafts each section from a bounded, validated fact set with STRICT
  citation discipline (every figure must reference a supplied fact id; unseen ids are
  dropped). Persists `content.sections`, `content.facts` (in-scope facts) and
  `content.scope` on the `Report` row. LLM down → falls back to a fact-only bullet body.
- `exporter.py` — renders a structured, professional PDF (reportlab, no new deps):
  1. **Cover block** — CoalSutra brand, title, template, color-coded status
     (draft/reviewed/final), generated timestamp, entity/period scope, fact count.
  2. **Key Metrics** — auto table of every in-scope fact: entity / value / unit / date /
     source document / page / confidence, zebra-striped, capped at 40 rows.
  3. **Insight & Analysis** — computed DETERMINISTICALLY from the facts (never invented):
     first→latest value per entity, absolute + % change, trend direction (up/down/flat),
     min/max, source coverage, "widest coverage" highlight, and native vector **bar
     charts** per numeric series with ≥ 2 dated points (up to 2 charts).
  4. Drafted narrative sections (LLM prose, still cited) + **References / Sources**
     appendix (every cited document + page).
  - Page furniture: header strip (brand + title), footer with template + page number.
...
9. **Tests** — `pytest` green: **88 passing** (hermetic: forced no-LLM + hash
